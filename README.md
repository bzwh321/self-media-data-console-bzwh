...
2. 用普通中文逐项问我：使用哪些平台、每个平台的公开账号别名、看板展示名称、是否已有采集服务器，以及是否需要月度涨粉或收入目标。不要让我填写平台代码或 JSON 字段。
3. 根据我的回答，从 config.example.json 生成仅保存在本机的 config.json，切换到 user 模式，并创建对应的 data/user 平台目录。如果已有采集服务器，继续询问 SSH 主机、本机密钥文件路径和各平台远端数据目录，替我配置 server_sync；只引用密钥路径，不读取或复制密钥内容。
...
7. 需要安装项目依赖时先说明用途；得到允许后再安装。随后直接验证服务器拉取、规范化、契约校验、紧凑数据生成和启动流程。
...
- 如果已有采集服务器：SSH 主机、本机密钥文件路径，以及各平台在服务器上的数据目录；不需要提供密钥内容。
...
7. 点击“刷新”会从已配置服务器拉取数据，完成规范化、契约校验和 T-1 新鲜度检查后才更新看板。

## 刷新按钮会做什么

- 模拟数据模式：重新生成截止到当天的模拟数据，并运行契约与新鲜度检查。
- 个人数据模式：从 `config.json` 配置的服务器目录拉取数据，然后依次运行规范化、业务契约检查、紧凑看板生成、归因分析和日报生成。
- 任一步失败：页面显示失败原因，不会把缓存数据提示为“刷新成功”。个人模式默认要求启用平台的数据不超过 T-1，可由 AI 在私有配置中调整。

## 隐私边界

- 登录、扫码、验证码和导出确认由用户本人完成。
- 不向 AI 提供密码、Cookie、验证码、浏览器 Profile、平台登录态、token 或 SSH 密钥内容；服务器同步只引用用户授权的本机密钥路径。
